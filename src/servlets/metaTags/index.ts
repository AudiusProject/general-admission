...
const getResponse = async (
  ...
  const baseUrl = "http://localhost:8000"; //process.env.PUBLIC_URL || "http://localhost:8000";
  switch (format) {
    case MetaTagFormat.Track:
      context.image = `${baseUrl}/og-image/track/${handle}/${title}`;
      break;
    case MetaTagFormat.Collection:
      context.image = `${baseUrl}/og-image/collection/${handle}/${title}`;
      break;
    case MetaTagFormat.User:
      context.image = `${baseUrl}/og-image/user/${handle}`;
      break;
    default:
      context.image = `${baseUrl}/og-image/default`;
  }
